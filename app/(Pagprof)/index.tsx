...
import { View, Image, Text, TextInput, Pressable, ScrollView, ActivityIndicator, Alert  } from "react-native";
...
async function deleteQuestao(id:string) {
    try {
        const response = await apiConfig.delete(`/questoes/${id}`);
        if (response.status === 200) {
            // Remove a questão da lista local
            setQuestoes((prevQuestoes) => prevQuestoes.filter((questao) => questao.id !== id));
        } else {
            console.error("Erro ao excluir a questão:", response);
        }
    } catch (error) {
        console.error("Erro ao excluir a questão:", error);
    }
}
function confirmDeleteQuestao(id:string) {
    Alert.alert(
        "Confirmar Exclusão",
        "Você tem certeza de que deseja excluir esta questão?",
        [
            {
                text: "Cancelar",
            ...
            {
                text: "Excluir",
                style: "destructive", // Destaca o botão de exclusão no iOS
                onPress: () => deleteQuestao(id), // Chama a função de exclusão
            },
        ]
    );
}
                    ...
                    {questoes.map((questao) => (
                        ...
                        <TextoQuestao>Questão: {questao.enunciado}</TextoQuestao>
                                <Acoes>
                                    ...
                                    <BotaoAcaoExcluir onPress={() => confirmDeleteQuestao(questao.id)}>
